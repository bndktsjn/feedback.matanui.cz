...
import AccessSetupModal from './o/[orgSlug]/projects/components/AccessSetupModal';
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const [projectDetail, setProjectDetail] = useState<{ baseUrl?: string } | null>(null);
  ...
  return (
    <div className="flex h-screen">
            ...
            <>
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    const detail = await projects.get(projectCtx.orgId, projectCtx.id) as { baseUrl?: string };
                    setProjectDetail(detail);
                    setWsModalOpen(true);
                  } catch {
                    setProjectDetail({ baseUrl: '' });
                    setWsModalOpen(true);
                  }
                }}
      ...
      {/* Access Setup Modal (same as Feedback button) */}
      {wsModalOpen && projectCtx && (
        <AccessSetupModal
          projectName={projectCtx.name}
          projectBaseUrl={projectDetail?.baseUrl || ''}
          projectSlug={projectCtx.slug}
