...
export class ProjectMemberGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // If not a direct project member, check org-level admin/owner → implicit project admin
    if (!membership) {
      const orgMember = await this.prisma.organizationMember.findUnique({
        where: { orgId_userId: { orgId: project.orgId, userId: user.id } },
      });
      if (orgMember && (orgMember.role === 'owner' || orgMember.role === 'admin')) {
        // Org owner/admin gets implicit project admin access
        (request as Request & { projectRole: string }).projectRole = 'admin';
        (request as Request & { project: typeof project }).project = project;
        return this.checkRoles(context, 'admin');
      }
